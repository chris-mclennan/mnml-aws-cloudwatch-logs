    ...
    loop {
        ...
        loop {
            match input_rx.try_recv() {
                ...
                // Newer protocol variants (Focus / Hover / Ime / …) are not
                // consumed by this viewer — ignore them like Mouse above.
                Ok(_) => {}
                Err(TryRecvError::Empty) => break,
                Err(TryRecvError::Disconnected) => return Ok(()),
            }
        ...
        // Pick up a live theme switch in mnml before we recolour cells.
